...
//Work of reciver ends here
	...
	{   
		...
		$postdata = file_get_contents("php://input");
                ...
                $file = substr($file , 2);
		$log->debug("reciver","Post Data Recived");	
		return $file;
	}
	...
	//json parsing
			...
			switch (json_last_error()) {
			        ...
			        case JSON_ERROR_NONE:
			        	$log->debug("reciver","No json errors");
			        	
			       //   echo ' - No json errors';
			       		break;
			        
			        case JSON_ERROR_DEPTH:
			        	$log->error("reciver","Maximum stack depth exceeded");
			        	$json_error = "Maximum stack depth exceeded";
			        	break;
			        
			        case JSON_ERROR_STATE_MISMATCH:
			        	$log->error("reciver","Underflow or the modes mismatch");

			            $json_error = "Underflow or the modes mismatch";
			        	break;
			        
			        case JSON_ERROR_CTRL_CHAR:
			        	$log->error("reciver","Unexpected control character found");
			            $json_error = "Unexpected control character found";
			        	break;
			        
			        case JSON_ERROR_SYNTAX:
			        	$log->error("reciver","Syntax error, malformed JSON");
			            $json_error = "Syntax error, malformed JSON";
			        	break;
			        
			        case JSON_ERROR_UTF8:
			        	$log->error("reciver","Malformed UTF-8 characters, possibly incorrectly encoded");
			            $json_error = "Malformed UTF-8 characters, possibly incorrectly encoded";
			        	break;
			        
			        default:
			        	$log->error("reciver","Unknown json Error");
			...
			//Check file type is json or not
 			if((json_last_error() == JSON_ERROR_NONE)==1){
				...
				$log->debug("reciver","Module Identified - $decoded_string->module"); 
	...
	{	
		...
		if(strcmp($module, "generate_timeline")== 0){
				$log->debug("reciver","Calling Generate Timeline Module Manager");
				...
				$log->debug("reciver","Calling Store Timeline Data Module Manager");
	    		...
	    		$log->debug("reciver","Calling Contacts Manager Module Manager");
				...
				$log->debug("reciver","Calling Music Module Manager");
			...
			$log->error("reciver","Module Manager Not Found");
